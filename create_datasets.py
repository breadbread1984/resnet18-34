...
class ImageNet(object):
  ...
  def train_parse_function(self, img, label):
    ...
    sample_x = tf.random.uniform(minval = tf.zeros_like(sample_h), maxval = tf.math.maximum(0, height - sample_h + 1), shape = (), dtype = tf.int32); # sample_x.shape = (10,)
    sample_y = tf.random.uniform(minval = tf.zeros_like(sample_w), maxval = tf.math.maximum(0, width - sample_w + 1), shape = (), dtype = tf.int32); # sample_y.shape = (10,)
